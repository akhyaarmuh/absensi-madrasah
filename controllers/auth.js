...
import { errorValidation, backupDatabase } from '../utilities/mongoose.js';
...
export const register = async (req, res) => {
  ...
  try {
    ...
    const newUser = new User({ ...req.body, role, status: role === 'admin' ? 1 : 0 });
    ...
    if (error.name === 'ValidationError')
      res
        .status(400)
        .json({ message: 'Data tidak benar', error: errorValidation(error) });
  ...
  try {
    ...
    if (!user)
      throw {
        ...
        errors: { email: { message: 'Pengguna tidak ditemukan' } },
      };
    ...
    if (!(await user.checkPassword(password)))
      throw {
        ...
        errors: { password: { message: 'Katasandi yang anda gunakan salah' } },
      };
    ...
    if (!user.status)
      throw {
        ...
        errors: { email: { message: 'Akun tidak aktif, silakan hubungi cs kami' } },
      };
